...
def test_alert_incident_ai_workflow(client, accounts):
    ...
    assert (
        client.patch(
            "/api/incidents/" + iid, headers=accounts["analyst"], json={"status": "Resolved", "resolution": "Approved maintenance confirmed."}
        ).status_code
        == 200
    )
...
def test_integrations(client, accounts):
    payload = {
        ...
        "src_ip": "203.0.113.7",
        "dest_ip": "10.0.0.4",
        "alert": {"signature": "Test signature", "severity": 1},
    }
    ...
    assert (
        client.post("/api/events/import/suricata", content=json.dumps(payload) + "\n" + json.dumps(payload), headers=accounts["analyst"]).json()["duplicates"]
        == 2
    )
    assert (
        client.post(
            "/api/events/import/wazuh",
            json={"timestamp": now().isoformat(), "rule": {"level": 10, "description": "Test sensor alert"}, "agent": {"name": "wazuh-test", "ip": "10.0.0.6"}},
            headers=accounts["analyst"],
        ).status_code
        == 201
    )
