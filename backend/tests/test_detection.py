...
def event(kind='ssh_failed',offset=0,**kwargs):
    return {'event_id':str(uuid4()),'timestamp':(now()-timedelta(minutes=2)+timedelta(seconds=offset)).isoformat(),'hostname':'test-server','source_ip':'198.51.100.25','destination_ip':'10.0.0.2','event_type':kind,'username':'root',**kwargs}
...
@pytest.mark.parametrize('score,label',[(0,'informational'),(20,'informational'),(21,'low'),(40,'low'),(41,'medium'),(60,'medium'),(61,'high'),(80,'high'),(81,'critical'),(100,'critical')])
...
def test_rules_reject_unsafe_and_unsupported_syntax():
    ...
    for text in ['!!python/object/apply:os.system [echo bad]', definition.replace('"condition": "selection"','"condition": "selection or anything"'), definition.replace('"event_type"','"event_type|regex"')]:
