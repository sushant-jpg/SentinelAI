    ...
    for stored in rules:
        ...
        if kind == "success_after_failures":
            ...
            evidence = [
                item
                for item in history
                if item.event_type in ("ssh_failed", "login_failed") and item.username == event.username and utc(item.timestamp) <= utc(event.timestamp)
            ]
        if kind == "unusual_source":
            if not event.username:
                continue
            previous = [
                item
                for item in history
                if item.event_type == "login_success" and item.username == event.username and utc(item.timestamp) < utc(event.timestamp)
            ]
        ...
        if kind != "single":
            recent = db.scalar(
                select(Alert)
                .where(
                    Alert.detection_rule == stored.id,
                    Alert.source_ip == event.source_ip,
                    Alert.affected_host == event.hostname,
        ...
        prior = (
            db.scalar(
                select(func.count(func.distinct(Incident.incident_id)))
                .select_from(Incident)
                .join(IncidentAlert)
                .join(Alert)
                .where(Alert.source_ip == event.source_ip)
            )
            if event.source_ip
            else 0
        )
        ...
        alert = Alert(
            ...
            detection_rule=stored.id,
            technique_id=options["technique_id"],
            evidence={
                "event_ids": ids,
                "matching_count": len(evidence),
