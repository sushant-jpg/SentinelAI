...
def refresh(request: Request, response: Response, db: Session = Depends(get_db)):
    ...
    db.rollback()
    changed = db.execute(update(AuthSession).where(AuthSession.id == session.id, AuthSession.refresh_hash == old_hash, AuthSession.revoked.is_(False)).values(refresh_hash=new_hash))
