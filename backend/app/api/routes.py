...
from app.models.entities import User, Event, Alert, Asset, DetectionRule, MitreTechnique, AnalystNote, Incident, IncidentAlert, IncidentTimeline, AuditLog, AuthSession, now
...
def events_list(page: int = Query(1, ge=1), page_size: int = Query(25, ge=1, le=100), event_id: str | None = None, db: Session = Depends(get_db), user: User = Depends(current_user)):
...
def alerts_list(page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100), severity: Severity | None = None, status: Status | None = None, q: str = Query('', max_length=200), ip: str | None = None, host: str | None = None, username: str | None = None, event_type: str | None = None, technique: str | None = None, min_risk: int = Query(0, ge=0, le=100), max_risk: int = Query(100, ge=0, le=100), start: datetime | None = None, end: datetime | None = None, db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    for field, value in [(Alert.severity,severity),(Alert.status,status),(Alert.source_ip,ip),(Alert.affected_host,host),(Alert.username,username),(Alert.event_type,event_type),(Alert.technique_id,technique)]:
        if value:
            query = query.where(field == value)
    if q:
        query = query.where(or_(*(field.icontains(q, autoescape=True) for field in [Alert.title,Alert.source_ip,Alert.affected_host,Alert.username,Alert.technique_id])))
    if start:
...
def alert_detail(alert_id: str, db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    result['events'] = [serialize(e) for e in db.scalars(select(Event).where(Event.event_id.in_(alert.evidence['event_ids'])).order_by(Event.timestamp).limit(100))]
    return result
...
def create_incident(body: IncidentCreate, request: Request, db: Session = Depends(get_db), user: User = Depends(analyst)):
    ...
    incident = Incident(title=body.title, description=body.description, severity=max(alerts,key=lambda a:a.risk_score).severity, assigned_analyst=body.assigned_analyst, affected_assets=sorted({a.affected_host for a in alerts}))
    db.add(incident)
    db.flush()
    for alert in alerts:
...
def incident_detail(incident_id: str, db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    result['timeline'] = [serialize(t) for t in db.scalars(select(IncidentTimeline).where(IncidentTimeline.incident_id == incident_id).order_by(IncidentTimeline.timestamp))]
    return result
...
def update_incident(incident_id: str, body: IncidentPatch, request: Request, db: Session = Depends(get_db), user: User = Depends(analyst)):
    ...
    if incident.status == 'Resolved' and body.resolution is not None and not body.resolution.strip() and body.status not in ('New','Investigating','False Positive'):
    ...
    db.add(IncidentTimeline(incident_id=incident_id, user_id=user.id, action=body.status or 'Updated', note=body.note or body.resolution or 'Assignment or incident details updated.'))
...
def assets_list(db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    for asset in db.scalars(select(Asset).order_by(Asset.hostname)):
        count, risk = db.execute(select(func.count(),func.max(Alert.risk_score)).where(Alert.affected_host == asset.hostname, Alert.status.in_(['New','Investigating']))).one()
        items.append({**serialize(asset), 'number_of_alerts':count, 'risk_score':risk or 0, 'agent_status':'active' if (now()-utc(asset.last_seen)).total_seconds() < 900 else 'stale'})
    return {'items':items}
...
def settings(user: User = Depends(current_user)):
    cfg = get_settings()
    return {'demo_mode':cfg.demo_mode, 'registration_enabled':cfg.registration_enabled, 'environment':cfg.environment, 'ai_provider':cfg.ai_provider, 'integrations':{'wazuh':'Import API available', 'suricata':'EVE JSON import available'}, 'access_token_minutes':15, 'rule_format':'Sigma subset + SentinelAI correlations'}
