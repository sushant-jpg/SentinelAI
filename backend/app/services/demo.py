...
def demo_events():
    ...
    def add(offset, host="gateway-prod", source="198.51.100.24", **fields):
        events.append(
            EventInput(
                ...
                hostname=host,
                source_ip=source,
                destination_ip="10.20.0." + str(10 + len(host)),
                **fields,
            )
        )
    ...
    for hour in range(24):
        ...
        for i in range(12):
            add(
                offset + i * 3,
                host=["gateway-prod", "auth-server", "db-primary"][hour % 3],
                source=f"198.51.100.{24 + hour % 5}",
                event_type="ssh_failed",
                username="admin",
                destination_port=22,
                message="Simulated sshd: authentication failure",
            )
        ...
        if hour % 3 == 0:
            for port in range(20, 44):
                add(
                    ...
                    protocol="TCP",
                    message="Simulated network connection record",
                )
        if hour % 4 == 0:
            ...
            add(
                ...
                message="Simulated log: powershell -EncodedCommand [inert sample]",
            )
        if hour % 5 == 0:
            add(
                offset + 210,
                ...
                severity="critical",
                message="SIMULATED suspicious outbound application protocol signature",
            )
            for i in range(6):
                add(
                    offset + 230 + i,
                    host="auth-server",
                    source="198.51.100.101",
                    event_type="login_failed",
                    username=f"user{i}",
                    message="Simulated account authentication failure",
                )
    return events
