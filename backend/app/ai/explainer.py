...
class LocalExplainer:
    def explain(self, alert: dict) -> dict:
        return {
            "provider": "local",
            "generated_by": "Deterministic explanation template",
            ...
            "investigation": [
                "Review the referenced event records and their timestamps.",
                "Confirm whether the activity matches approved maintenance or testing.",
                "Correlate authentication, endpoint, and network logs; check for successful access.",
            ],
            "recommendations": [
                "Preserve relevant logs and document findings.",
                "If confirmed and authorized, restrict the source or isolate the affected asset.",
                "Review account privileges, enforce MFA, and tune the rule after validation.",
            ],
            ...
            with httpx.Client(timeout=15, follow_redirects=False) as client:
                response = client.post(
                    ...
                    json={
                        ...
                        "messages": [{"role": "system", "content": prompt}, {"role": "user", "content": json.dumps(summary)}],
                ...
                if not isinstance(data, dict):
                    raise ValueError("Expected a provider response object")
                if (
                    not isinstance(data.get("interpretation"), str)
                    or len(data["interpretation"]) > 5000
                    or any(
                        not isinstance(data.get(k), list) or len(data[k]) > 10 or not all(isinstance(v, str) and len(v) <= 2000 for v in data[k])
                        for k in ("investigation", "recommendations")
                    )
                ):
                ...
                result.update(
                    provider="external",
                    generated_by="External AI — unverified guidance",
                    limitations="AI-generated interpretations and recommendations may be incorrect. Evidence is supplied only by the detection engine.",
                )
...
def explain(alert: dict) -> dict:
    settings = get_settings()
    provider = (
        RemoteExplainer() if settings.ai_provider == "external" and settings.ai_api_url.startswith("https://") and settings.ai_api_key else LocalExplainer()
    )
