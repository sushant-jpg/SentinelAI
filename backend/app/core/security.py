...
def issue_tokens(db: Session, user: User, session: AuthSession | None = None):
    ...
    token = jwt.encode({'sub': user.id, 'sid': session.id, 'iat': now(), 'exp': now() + timedelta(minutes=15), 'iss': 'sentinelai', 'aud': 'sentinelai-ui', 'type': 'access'}, get_settings().jwt_secret, algorithm='HS256')
...
def current_user(request: Request, credentials: HTTPAuthorizationCredentials | None = Depends(bearer), db: Session = Depends(get_db)) -> User:
    ...
    try:
        claims = jwt.decode(credentials.credentials, get_settings().jwt_secret, algorithms=['HS256'], audience='sentinelai-ui', issuer='sentinelai', options={'require': ['exp', 'iat', 'sub', 'sid', 'type']})
