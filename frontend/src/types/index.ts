...
export interface Alert {
  ...
  risk_factors: { factor: string; points: number }[];
  notes?: { id: string; text: string; user_id: string; timestamp: string }[];
  technique?: { tactic: string; technique_name: string };
...
export interface Analytics {
  ...
  total_events: number;
  trend: { date: string; total: number; critical: number; high: number }[];
  categories: Count[];
  top_ips: Count[];
  top_hosts: Count[];
  top_users: Count[];
