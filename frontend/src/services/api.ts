...
export async function restoreSession(): Promise<User | null> {
  if (!refreshPromise) refreshPromise = fetch('/api/auth/refresh', {method:'POST',credentials:'include'}).then(async response => {
    if (!response.ok) {token=null; return null}
    const data = await response.json(); token = data.access_token; return data.user as User
...
export async function api<T>(path: string, options: RequestInit = {}, retry = true): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(typeof data.detail === 'string' ? data.detail : Array.isArray(data.detail) ? data.detail.map((e:{msg:string}) => e.msg).join('; ') : `Request failed (${response.status})`)
  }
